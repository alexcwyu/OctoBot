...
:::info
  Warning: The automation of TradingView alerts by email is unfortunately no longer available due to a recent restriction by TradingView.
:::
...
:::info
  The [Premium OctoBot
  Extension](../../octobot-configuration/premium-octobot-extension) is required
  to trade with OctoBot using TradingView email alerts.
:::

## TradingView email alerts configuration

To automate your trades based on TradingView alerts using emails, simply go to your `Accounts` configuration, select the `Interfaces` tab and add the `TradingView` interface.
...
Now that you received your personal TradingView alert email address, simply follow the quick configuration steps directly from your OctoBot to add this email address to your TradingView alerts.
...
Your OctoBot is now ready to trade using your TradingView alerts. You can trade on any market and exchange following the [OctoBot open source alert format](alert-format).
