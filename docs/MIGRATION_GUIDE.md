...
- **Build backend**: `hatchling` (was: `hatchling` - kept, metadata modernized)
...
### 3. Source Layout
- **Layout**: `src/` layout
- **Package moved**: `octobot/` -> `src/octobot/`
- **Entry point moved**: `start.py` -> `src/start.py`
- **Import unchanged**: `import octobot` still works

If upstream adds files to the old location, move them to `src/octobot/`.
...
### 6. Known Issues
- **octobot-tulipy** (v0.4.10): Transitive dependency via `octobot-evaluators` that fails to compile its Cython extensions on Python 3.13. `uv sync` will fail until upstream releases a 3.13-compatible version. Workaround: `uv pip install -e . --no-deps` then install compatible deps manually, or wait for upstream fix.
...
3. **Check source layout**: If upstream adds new modules to the old path, move them to `src/octobot/`.
4. **Re-lock**: Run `uv lock` to update `uv.lock` with any new/changed dependencies.
5. **Verify**: Run `uv sync && uv run python -c "import octobot" && uv run pytest` (if tests exist).

## Quick Commands

```bash
uv sync                                    # Install all deps
uv run python -c "import octobot"          # Verify import
